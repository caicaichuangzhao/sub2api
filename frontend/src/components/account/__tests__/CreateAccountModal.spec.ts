...
function mountModal() {
  return mount(CreateAccountModal, {
    props: { show: true, proxies: [], groups: [], batches: [{ id: 1, name: 'Test batch' }] },
    global: {
      stubs: {
...
async function selectTestBatch(wrapper: ReturnType<typeof mountModal>) {
  const batchSelect = wrapper
    .findAll('select')
    .find(select => select.findAll('option').some(option => option.text() === 'Test batch'))
  expect(batchSelect).toBeDefined()
  await batchSelect?.setValue('1')
}
