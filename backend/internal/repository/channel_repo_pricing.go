...
func scanModelPricingRows(rows *sql.Rows) ([]service.ChannelModelPricing, []int64, error) {
	...
	for rows.Next() {
		...
		if err := unmarshalTimePricing(timePricingJSON, &p.TimePricing); err != nil {
...
func unmarshalTimePricing(value []byte, target **service.TimePricingConfig) error {
	if len(value) == 0 || string(value) == "null" {
		*target = nil
		return nil
	}
	var config service.TimePricingConfig
