...
func (r *accountRepository) Create(ctx context.Context, account *service.Account) error {
	...
	// batch_id is managed by the custom batch migration and is not in the Ent schema.
	if account.BatchID != nil {
		if _, err := r.sql.ExecContext(ctx, `UPDATE accounts SET batch_id = $1 WHERE id = $2`, *account.BatchID, account.ID); err != nil {
			return err
		}
	}
...
func (r *accountRepository) updateAccount(ctx context.Context, account *service.Account, explicitProbeEnabled *bool) error {
	...
	if account.BatchID != nil {
		if *account.BatchID > 0 {
			_, err = client.ExecContext(ctx, `UPDATE accounts SET batch_id = $1 WHERE id = $2`, *account.BatchID, account.ID)
		} else {
			_, err = client.ExecContext(ctx, `UPDATE accounts SET batch_id = NULL WHERE id = $1`, account.ID)
		}
		if err != nil {
			return err
		}
